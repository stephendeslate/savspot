...
export class PublicBookingController {
  ...
  @Public()
  @ApiOperation({
    summary: 'Get tenant public profile for preview mode (no real bookings)',
  })
  @ApiResponse({
    status: 200,
    description: 'Tenant public profile with preview flag',
  })
  @ApiResponse({ status: 404, description: 'Business not found' })
  async getTenantBySlugPreview(@Param('slug') slug: string) {
    const tenant = await this.publicBookingService.getTenantBySlug(slug);
