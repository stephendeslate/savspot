...
| FR-PAY-11 | Platform referral commission calculation | Must | 1 |
...
| FR-PAY-17 | Subscription billing via Stripe Billing: plan creation (per-feature pricing), upgrade/downgrade with proration, automated recurring invoicing, payment method management | Must | 2 |
...
| FR-COM-2a | SMS notifications to the business owner/provider for critical real-time events (via Twilio): new booking received, booking cancelled by client, booking rescheduled. Content format: "{Client} booked {Service} at {Time} on {Date}." Provider SMS is Phase 1 because real-time provider awareness is required for the parallel-run design partner scenario — without it, SavSpot feels silent compared to Booksy's native push notifications. Low volume (3–5 bookings/week for soft launch). | Must | 1 |
| FR-COM-2b | SMS notifications to clients (via Twilio): booking confirmation, 24h reminder, 2h reminder. Shipped with full SMS template system in Phase 2 alongside advanced email (FR-COM-1b). | Must | 2 |
| FR-COM-3 | Email template engine with variable substitution | Must | 2 |
| FR-COM-4 | Automated triggers: confirmed, cancelled, 24h reminder, 24h follow-up, post-appointment review request (2h after COMPLETED, email + SMS Phase 2), rebooking prompt (FR-BFW-19, included in follow-up email Phase 1) | Must | 1 |
...
| FR-CRM-28 | One-tap quick actions in calendar: from the appointment list view (FR-CRM-27), each appointment displays contextual quick actions achievable in a single tap without navigating away: (a) Mark as arrived (sets `check_in_status = CHECKED_IN` — pre-Phase 2 check-in state machine, records timestamp); (b) Mark as completed (triggers CONFIRMED → COMPLETED transition); (c) Quick Add Walk-In (opens a bottom sheet: select service → optionally select/create client → confirm; creates a WALK_IN booking directly as CONFIRMED, per FR-BFW-18). These three actions cover the most frequent staff interactions during a working day. | Should | 1 |
...
| ID | Requirement | Pri | Phase |
|----|------------|-----|-------|
| FR-MOB-1 | Business booking page viewing with services | Must | 2 |
| FR-MOB-2 | Booking flow wizard (core steps: service selection, date/time, pricing summary, payment, confirmation). Full parity with web advanced steps (guest config age tiers, questionnaires, contracts, add-ons) ships alongside those web features in Phase 2. | Must | 2 |
| FR-MOB-3 | Client portal: view bookings, booking history, booking detail, payment management, messaging | Must | 2 |
| FR-MOB-4a | Basic push notifications (booking confirmation, reminders) | Must | 2 |
| FR-MOB-4b | Push notifications with per-channel config via notification system (aligns with FR-NOT-2) | Must | 2 |
| FR-MOB-5 | Biometric authentication (Face ID / Fingerprint) | Must | 2 |
| FR-MOB-6 | Secure token storage (Keychain / Keystore) | Must | 2 |
| FR-MOB-7 | Offline mode: view saved bookings and businesses | Should | 2 |
| FR-MOB-8 | Deep linking for direct navigation | Must | 2 |
| FR-MOB-9 | Deep link support for booking pages | Must | 2 |
| FR-MOB-10 | Review submission with photos | Should | 2 |

> **Mobile app deferred to Phase 2.** The native mobile app is deferred from Phase 1 to Phase 2 for three reasons: (1) mobile-responsive web (FR-BP-5) covers all client booking scenarios during initial validation — clients can browse, book, pay, and manage bookings from any mobile browser; (2) deferral reclaims ~2–3 weeks of Phase 1 development time (React Native + Expo setup, biometric auth, push notification infrastructure, EAS Build pipeline, Maestro E2E tests), accelerating time-to-validation; (3) Phase 2 timing is actually better for the mobile app, as it ships alongside the full notification system (FR-NOT-1 through FR-NOT-5), push notification per-channel config (FR-MOB-4b), check-in/check-out (FR-CRM-23), and reviews (FR-CRM-24), all of which benefit from native capabilities. Code sharing via `packages/shared` (Zod schemas, types, constants, business logic) minimizes incremental effort when mobile development begins. LifePlace demonstrated production React Native shipping capability with the same developer and tooling. Admin CRM remains web-only in all phases.
