# SavSpot Current State — March 7, 2026
...
`BookingSessionsService` now has 5 constructor dependencies:
1. `PrismaService`
2. `ReservationService`
3. `PaymentService`
4. `BookingEventsService`
5. `ConsentService` (new)

Tests that construct this service must provide all 5 (as `never` casts for mocks).
...
|-------|--------|----------------|
| Node.js v18 → v22 upgrade | Pending | `nvm install 22` (requires sudo on EVO-X2) |
...
6 warnings about unused `eslint-disable` directives in existing code. Not introduced by this changeset — pre-existing.
